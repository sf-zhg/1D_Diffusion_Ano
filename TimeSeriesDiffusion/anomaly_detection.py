...
def detect_anomalies(
    ...
    for i in range(num_batches):
        ...
        #print("Delta shape:", delta_og_gen.shape)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(csv_path, f"anomaly_plot_Epoch_{epoch}_Label_{y}_sample{i}.png"))
        plt.close()
        ...
        ax.set_xlabel("Time Steps")
        ax.set_ylabel("Values")
        ax.set_title(f"Original and Generated Series with Anomalies (Sample {i})")
        ax.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(csv_path, f"anomaly_plot_Epoch_{epoch}_Label_{y}_sample{i}.png"))
        plt.close()'''
