...
def forward_diffusion_stepwise(
    ...
    for i in tqdm(range(0, diffusion_model.time_steps),
                  desc = 'sampling loop time step',
                  total = diffusion_model.time_steps):
        t = torch.full((1,), i, device = device)
        noisy_series_t = diffusion_model.q_sample(x_start = x,
                                                  t = t,
                                                  base_noise = noise)
...
def save_to_csv(
    ...
    for i in range(gen_size):
        ...
        df_generated.to_csv(
            f"{gen_sample_file_path}_{i}.csv", index=False, header=True
        )
    ...
    for i in range(noisy_size):
        ...
        df_noisy.to_csv(
            f"{noisy_sample_file_path}_{i}.csv", index=False, header=True
        )
...
def plot_time_series(
    ...
    for i in range(num_timesteps_generated):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(csv_path, f"gen_plot_epoch_{epoch}_class_{y}_sample_{i}.png"))
        plt.close()
    ...
    for i in range(num_timesteps_noisy):
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(csv_path, f"noisy_plot_epock_{epoch}_class_{y}_sample_{i}.png"))
        plt.close()
